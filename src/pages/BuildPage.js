...
const BuildPage = () => {
  ...
  let buildProjects = projects.filter(project => 
    buildCategories.includes(project.category)
  )

  // Apply additional filters
  if (selectedYear) {
    buildProjects = buildProjects.filter(project => project.year === selectedYear)
  }
  if (selectedLocation) {
    buildProjects = buildProjects.filter(project => project.location === selectedLocation)
  }
  if (selectedCategory) {
    buildProjects = buildProjects.filter(project => project.category === selectedCategory)
  }

  // Get unique values for filter options
  const availableYears = [...new Set(projects.filter(p => buildCategories.includes(p.category)).map(p => p.year))].sort()
  const availableLocations = [...new Set(projects.filter(p => buildCategories.includes(p.category)).map(p => p.location))].sort()
  const availableCategories = [...new Set(projects.filter(p => buildCategories.includes(p.category)).map(p => p.category))].sort()
